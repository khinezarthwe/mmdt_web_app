            ...
            for question in current_page_questions:
                ...
                if question.question_type == Question.TEXT:
                    ...
                    self.fields[field_name] = forms.CharField(
                        label=question.question_text, required=False, initial=initial_value)
                    ...
                    self.fields[field_name] = forms.ChoiceField(choices=choices, label=question.question_text, widget=forms.RadioSelect, required=False, initial=initial_value)
                elif question.question_type == Question.CHECKBOX:
                    choices = [(choice.id, choice.choice_text) for choice in question.choices.all()]
                    initial_values = [r.choice_id for r in response_by_question.get(question.id, [])]
                    self.fields[field_name] = forms.MultipleChoiceField(choices=choices, label=question.question_text, widget=forms.CheckboxSelectMultiple, required=False, initial=initial_values)
                elif question.question_type == Question.LONG_TEXT:
                    initial_value = response_by_question.get(question.id, [None])[0].response_text if response_by_question.get(question.id, [None])[0] else None
                    self.fields[field_name] = forms.CharField(widget=forms.Textarea, label=question.question_text, required=False, initial=initial_value)
                elif question.question_type == Question.DROPDOWN:
                    initial_value = response_by_question.get(question.id, [None])[0].choice_id if response_by_question.get(question.id, [None])[0] else None
                    choices = [(choice.id, choice.choice_text) for choice in question.choices.all()]
                    self.fields[field_name] = forms.ChoiceField(choices=choices, label=question.question_text, required=False, initial=initial_value)
                elif question.question_type == Question.SLIDING_SCALE:
                    choices = [(choice.id, choice.choice_text) for choice in question.choices.all()]
                    initial_value = response_by_question.get(question.id, [None])[0].choice_id if response_by_question.get(question.id, [None])[0] else None
                    self.fields[field_name] = forms.IntegerField(label=question.question_text, widget=forms.NumberInput(attrs={'type': 'range'}), required=False, initial=initial_value)
    return SurveyForm
